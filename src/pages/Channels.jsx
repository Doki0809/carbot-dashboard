...
export default function Channels() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Stats */}
      <div className="grid grid-cols-3 gap-2 float-in stagger-1">
        {[
          { label: 'Disponibles', value: 3, accent: '#3b82f6', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg> },
          ...
          { label: 'Desconect.', value: loading ? '—' : 3 - connectedCount, accent: '#f59e0b', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> },
        ].map(({ label, value, accent, icon }) => (
          <div
            key={label}
            className="group relative flex flex-col items-start p-3 sm:p-6 overflow-hidden transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_20px_40px_rgba(0,0,0,0.5)] cursor-default"
            style={{ background: '#0e1015', borderRadius: '1.5rem', border: '1px solid rgba(255,255,255,0.03)' }}
          >
            <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none"
                 style={{ background: `radial-gradient(circle at 50% 0%, ${accent}25 0%, transparent 70%)` }} />
            <div
              className="w-8 h-8 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl flex items-center justify-center mb-2 transition-transform duration-500 group-hover:scale-110 relative z-10"
              style={{ background: '#151720', border: '1px solid rgba(255,255,255,0.05)', color: accent }}
            >
              <div className="w-4 h-4 sm:w-6 sm:h-6">{icon}</div>
            </div>
            <p className="text-[8px] sm:text-[11px] font-black uppercase tracking-widest text-white/50 leading-tight mb-1 relative z-10">{label}</p>
            <p className="text-2xl sm:text-4xl font-[900] tracking-tighter text-white leading-none relative z-10">{value}</p>
          </div>
        ))}
      </div>
